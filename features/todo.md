...
- [x] **"Why" descriptions don't match the recommended restaurant** *(fixed)*
    - Rewrote `prompt_rank.txt` to separate name citation from candidate description; description must now reflect the candidate's own cuisine/vibe/price. Increased `max_tokens` 300 → 500.

- [x] **Session-only recommendations are off** *(fixed)*
    - `top_cuisine_types` from `build_taste_profile` now flows to `searchNearby` as `includedTypes` when `input_weight > 0.3`. Generic types (`restaurant`, `food`, etc.) are filtered out so Google receives a specific signal (e.g. `["mexican_restaurant"]`).
